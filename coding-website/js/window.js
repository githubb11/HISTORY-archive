...
function cc_close_all_topnav() {
  cc_close_nav("tutorials");
  cc_close_nav("references");
  cc_close_nav("exercises");
}
(function () {
...
function searchfield_focus(obj) {
  obj.style.color = "";
  obj.style.fontStyle = "";
  if (obj.value == "Search ccschools.com") {obj.value = "";}
}
...
function clickFBLike() {
  ...
  document.getElementById("popupDIV").innerHTML = "<iframe src='/fblike.asp?r=" + Math.random() + "' frameborder='no' style='height:200px;width:250px;'></iframe><br><button onclick='hideFBLike()' class='cc-btn cc-black'>Close</button>";
}
function hideFBLike() {
...
//        codemirrorize(x[i]);
//      } else {
        x[i].innerHTML = ccCodeColorize(x[i].innerHTML + " ", modes[j]);
//      }
    }
}
...
function ccCodeColorize(x, lang) {
  ...
  function htmlMode(txt) {
    ...
    php = new extract(rest, "&lt;\\?php", "?&gt;", phpMode, "ccPHPPOS");
    rest = php.rest;
    comment = new extract(rest, "&lt;!--", "--&gt;", commentMode, "ccHTMLCOMMENTPOS");
    rest = comment.rest;
    ...
    for (i = 0; i < comment.arr.length; i++) {
        rest = rest.replace("ccHTMLCOMMENTPOS", comment.arr[i]);
    }
    for (i = 0; i < php.arr.length; i++) {
        rest = rest.replace("ccPHPPOS", php.arr[i]);
    }
    return rest;
  }
  ...
  function cssMode(txt) {
    ...
    comment = new extract(rest, /\/\*/, "*/", commentMode, "ccCSSCOMMENTPOS");
    rest = comment.rest;
    while (rest.search("{") > -1) {
      s = rest.search("{");
    ...
    rest = rest.replace(/}/g, "<span class='cssdelimitercolor' style=color:" + cssdelimitercolor + ">}</span>");
    for (i = 0; i < comment.arr.length; i++) {
        rest = rest.replace("ccCSSCOMMENTPOS", comment.arr[i]);
    }
  ...
  function jsMode(txt) {
    ...
    for (i = 0; i < esc.length; i++) {
      rest = rest.replace("ccJSESCAPE", esc[i]);
    }
